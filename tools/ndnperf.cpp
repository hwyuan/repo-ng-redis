...
static const uint64_t DEFAULT_BLOCK_SIZE = 100;
...
private:
  ...
  uint64_t m_maxSegmentNo;

  typedef std::unordered_map<uint64_t, shared_ptr<ndn::Data> > DataContainer; // hash table: <sequence number, content>
...
NdnPutFile::prepareNextData(uint64_t referenceSegmentNo)
{
  ...
  if (!m_data.empty()) {
    ...
    uint64_t maxSegmentNo = m_maxSegmentNo;
  ...
  // std::cout << "nDataToPrepare = " << nDataToPrepare << std::endl;
  ...
  for (size_t i = 0; i < nDataToPrepare && !m_isFinished; ++i) {
    ...
    // Maximum number of packets to send, should be much larger
    // std::cout << "m_currentSegmentNo = " << m_currentSegmentNo << std::endl;
    if (m_currentSegmentNo == m_maxPacketNo) {
      m_isFinished = true;
    }
    ...
    // insert random data
    data->setContent(buffer, DEFAULT_BLOCK_SIZE);
...
// Prepares data using the Datacontainer structure (hash table)
void
NdnPutFile::onInterest(const ndn::Name& prefix, const ndn::Interest& interest)
{
  ...
  uint64_t segmentNo;
  try {
    // last component is segment number
  ...
  // std::cout << "segementNo = " << segmentNo << std::endl;
  ...
  if ( (segmentNo > 1) && (segmentNo % 1000000 == 0)) {
    std::cout << "m_maxPacketNo = " << segmentNo << std::endl;
    gettimeofday(&end, NULL);
    seconds  = end.tv_sec  - start.tv_sec;
    useconds = end.tv_usec - start.tv_usec;
    mtime = ((seconds) * 1000 + useconds/1000.0) + 0.5;
    printf("total_time =: %ld milliseconds, %ld seconds\n", mtime, seconds);
    printf("pps =: %ld\n", segmentNo / seconds);
  }

	if (segmentNo == m_maxPacketNo) {
		printf("finished... segmentNo = %d\n", segmentNo);
    exit(0);
	}
...
// Used only if it is specified in the command line argument
void
NdnPutFile::onSingleInterest(const ndn::Name& prefix, const ndn::Interest& interest)
{
  ...
  // Option 1: sign data packets
  // Option 2: return random data packets with specified length
...
main(int argc, char** argv)
{
  ...
  int opt;
  while ((opt = getopt(argc, argv, "usDi:I:n:x:l:w:vh")) != -1) {
    switch (opt) {
    case 'u':
    ...
    case 'I':
      ...
      break;
    case 'n':
      ndnPutFile.m_maxPacketNo = std::atoi(optarg);
      break;
    case 'x':
      try {
